...
def _MMR(embdistrib, text_obj, candidates, X, beta, N, use_filtered, alias_threshold):
    ...
    print('np.shape(sim_between_norm):', np.shape(sim_between_norm))
    selected_candidates = []
    ...
    for _ in range(N - 1):
        ...
        print('np.shape(selec_array):', np.shape(selec_array))
        print('np.shape(unselec_array):', np.shape(unselec_array))
        distance_to_doc = doc_sim_norm[unselec_array, :]
        dist_between = sim_between_norm[unselec_array][:, selec_array]
        print('np.shape(distance_to_doc):', np.shape(distance_to_doc))
        print('np.shape(dist_between):', np.shape(dist_between))
...
def MMRPhrase(embdistrib, text_obj, beta=0.65, N=10, use_filtered=True, alias_threshold=0.8):
    ...
    # print(candidates,X)
    print('embedding shape:',np.shape(X))
    # print(X)
    if len(candidates) == 0:
